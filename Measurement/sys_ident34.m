clc; clear;
data = load("exp1_034.mat");
int_g = load("est_tf_step.mat");
int_sys= int_g.sys;
time = data.exp1_034.X.Data;
Ts_arr = diff(time);
mean_diff = mean(Ts_arr);
std_diff = std(Ts_arr);
Ts = mean_diff;

inp_lvdt = data.exp1_034.Y(1).Data; % Amplifier out
out_lvdt = data.exp1_034.Y(2).Data; % LVDT Out
inp_ampf = data.exp1_034.Y(3).Data; % Inp to amplifier
